...
export async function collectCursor(io) {
    ...
    let data;
    try {
        ({status, data} = await io.http('https://api2.cursor.sh/aiserver.v1.DashboardService/GetCurrentPeriodUsage',
            {method: 'POST', headers: {Authorization: `Bearer ${token}`, 'Connect-Protocol-Version': '1'}, body: {}}));
    } catch {
        result.limits = {...result.limits, ...section('unavailable', 'Could not reach Cursor usage. Retry later.')};
        return result;
    }
    ...
    source: result.limits.windows.length ? 'Cursor dashboard' : null};
    return result;
}
